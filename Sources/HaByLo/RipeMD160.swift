public struct RipeMD160: Sendable {
    ...
    @usableFromInline
    var count: Int64  // Total # of bytes processed.
    ...
    mutating func compress(_ X: UnsafePointer<UInt32>) {
        ...
        func FF(
            _ a: inout UInt32, _ b: UInt32, _ c: inout UInt32, _ d: UInt32, _ e: UInt32,
            _ x: UInt32, _ s: UInt32
        ) {
            a = a &+ F(b, c, d) &+ x
            a = ROL(a, s) &+ e
            c = ROL(c, 10)
    ...
    @inlinable
    public mutating func update(data: ArraySlice<UInt8>) {
        ...
        if buffer.count > 0 && buffer.count + length >= 64 {
            ...
            length -= amount
            buffer.append(contentsOf: data[..<pos])
        ...
        while length >= 64 {
            let start = pos
            data.formIndex(&pos, offsetBy: 64)
            X.withUnsafeMutableBytes {
                $0.copyBytes(from: data[start..<pos])
            }
            compress(X)
            length -= 64
        }
    ...
    public mutating func finalize() -> [UInt8] {
        ...
        self = .init()
        return result.withUnsafeBytes { Array($0) }
    }
}
